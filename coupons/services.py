...
def apply_coupon_to_cart(cart, code):
    ...
    try:
        ...
        raise CouponError('This coupon code doesn’t exist.')

    if not coupon.is_valid_now():
        raise CouponError('This coupon code is no longer valid.')

    if coupon.minimum_order_amount and cart.total_price < coupon.minimum_order_amount:
        raise CouponError(f'This code requires a minimum order of QAR {coupon.minimum_order_amount:.2f}.')
...
def redeem_coupon_for_order(order, code):
    ...
    if not coupon.redemptions_left_for(order.email):
        raise CouponError('This coupon code has already been used on this email address.')
