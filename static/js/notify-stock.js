(function () {
  ...
  function markSubscribed(trigger) {
    ...
    if (trigger.dataset.label) {
      ...
      trigger.setAttribute('aria-label', "You're on the list — we'll email you");
      trigger.setAttribute('title', "You're on the list — we'll email you");
    }
  }
  ...
  function submitNotify(trigger, email) {
    ...
    fetch(url, {
      ...
      .then((res) => res.json())
      .then((data) => {
        if (window.ELinkCart) window.ELinkCart.showToast(data.message);
        if (data.success) {
      ...
      .catch(() => {
        trigger.disabled = false;
        if (window.ELinkCart) window.ELinkCart.showToast('Something went wrong. Please try again.');
      });
  }
